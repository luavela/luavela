...
static void inc_total_allocated_bytes(size_t size)
{
  __sync_fetch_and_add(&(get_stats()->active), size);
}

static void dec_total_allocated_bytes(size_t size)
{
  __sync_fetch_and_sub(&(get_stats()->active), size);
}
...
static LJ_AINLINE void *CALL_MMAP(size_t size)
{
  ...
  if (ptr != CMFAIL) {
    inc_total_allocated_bytes(size);
  }
  errno = olderr;
  return ptr;
}
...
static LJ_AINLINE int CALL_MUNMAP(void *ptr, size_t size)
{
  ...
  if (ret == 0) {
    dec_total_allocated_bytes(size);
  }
  errno = olderr;
  return ret;
}
...
static LJ_AINLINE void *CALL_MREMAP_(void *ptr, size_t osz, size_t nsz, int flags)
{
  ...
  if (ptr != MAP_FAILED) {
    if (osz < nsz) {
      inc_total_allocated_bytes(nsz-osz);
    } else {
      dec_total_allocated_bytes(osz-nsz);
    }
  }
  errno = olderr;
  return ptr;
...
/* page-align a size */
#define page_align(S)\
 (((S) + (UJ_PAGESIZE - SIZE_T_ONE)) & ~(UJ_PAGESIZE - SIZE_T_ONE))
...
static void *tmalloc_large(mstate m, size_t nb)
{
  ...
  if (t == 0 && v == 0) { /* set t to root of next non-empty treebin */
    ...
    if (leftbits != 0)
      t = *treebin_at(m, lj_ctz(leftbits));
  }
...
void *uj_alloc_f(void *msp, void *ptr, size_t osize, size_t nsize)
{
  (void)osize;
  if (nsize == 0) {
