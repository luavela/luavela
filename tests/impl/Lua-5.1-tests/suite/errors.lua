...
-- UJIT: parsing is different in the chunk below
-- checksyntax("break label", "", "label", 1)
-- UJIT: ";" is not a valid chunk
-- assert(doit";")
-- UJIT: ";;" is not a valid char sequence in the chunk
-- assert(doit"a=1;;")
assert(doit"return;;")
assert(doit"assert(false)")
assert(doit"assert(nil)")
-- UJIT: LuaJIT does not care about ambiguity in the chunk below
-- assert(doit"a=math.sin\n(3)")
...
-- UJIT: LuaJIT explicitly names variable in assertions below which is incosistent with the stock behavior
...
-- UJIT: Implementation of fast functions does not provide enough info on failure
...
-- UJIT: any character considered alphabetic by the current locale can be used in an identifier, but 0xff is not alphabetic
...
-- UJIT: skipped in favour of testrep_toocomplex
-- testrep("a=", "{")
testrep("a=", "(")
-- UJIT: skipped in favour of testrep_toocomplex
-- testrep("", "a(")
...
-- testing syntax limits: too complex expressions
-- UJIT: x2 encoding in uJIT
local function testrep_toocomplex (init, rep)
  local s    = "local a; " .. init .. string.rep(rep, 128)
  local a, b = loadstring(s)
  assert(not a and string.find(b, "too complex"))
end
testrep_toocomplex("a=", "{")
testrep_toocomplex("", "a(")
testrep_toocomplex("a=", "a..")
