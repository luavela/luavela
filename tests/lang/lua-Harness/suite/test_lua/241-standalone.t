...
-- UJIT: tests are too specific for standalone:
-- 1) Require info string to start from "Lua"
-- 2) Require "-b" flag to produce binary code (we currently produce human readable binary code)
-- So let's skip these tests for now
do
    skip_all("UJIT: tests are too specific for standalone")
end
